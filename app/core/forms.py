...
class CustomUserProfileModelAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        account_groups = list(filter(lambda x: x.name.startswith('account-'), chosen_groups))

        if len(account_groups) > 1:
            raise forms.ValidationError("Error in chosen group permissions! Only one account group allowed.")
